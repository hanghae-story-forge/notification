-- Migration: Add Multi-Tenant Support (Safe for existing data)
...
CREATE TYPE IF NOT EXISTS organization_member_status AS ENUM (
  'PENDING',
  'APPROVED',
  'REJECTED',
  'INACTIVE'
...
-- This allows us to make discord_id required later
DO $$
BEGIN
  -- Check if there are members with null discord_id
  IF EXISTS (SELECT 1 FROM members WHERE discord_id IS NULL) THEN
    -- Update null discord_id with a placeholder value
    UPDATE members
...
-- Now make discord_id unique (will fail if duplicates exist)
-- First, handle any duplicates by keeping the first occurrence
DO $$
DECLARE
...
BEGIN
  ...
  IF duplicate_count > 0 THEN
    RAISE NOTICE 'Found % duplicate discord_id values', duplicate_count;
    -- Note: Manual intervention may be needed to resolve duplicates
  END IF;
END $$;

-- Make discord_id unique if possible
DO $$
BEGIN
  -- Try to add unique constraint
  ALTER TABLE members ADD CONSTRAINT IF NOT EXISTS members_discord_id_key UNIQUE (discord_id);
EXCEPTION
  WHEN others THEN
    RAISE NOTICE 'Could not add unique constraint on discord_id (duplicates exist)';
END $$;

-- Make github optional if it exists and is NOT NULL
DO $$
BEGIN
  -- Drop unique constraint if exists
  ALTER TABLE members DROP CONSTRAINT IF EXISTS members_github_unique;

  -- Make column nullable if it's currently NOT NULL
  IF EXISTS (
    SELECT 1
